...
| Companion dismissal → goes home | ✅ **fixed 2026-07-28 (v6)** — reversed properly: `NPCStopFollowing#`/`NPCDespawn#` never move anyone; a dismissed companion returns only when their **TMX spawn object runs again** (`MonsterSpawn.Q()` re-uses the stored NPC and drops it on the spawn point). Janod's Kingsbridge spawn is gated `mad_wizard<100`, i.e. dead exactly when the mod recruits him; Grissenda's was fine but map state is cached ~1080 game-seconds. Fixed with `NPCSpawn#` homecoming triggers in `G9.tmx` (vanilla precedent: H10_mine/Teram). See `deobf/COMPANION_SPEC.md` §6 |
| Janod starting equipment | ✅ **added 2026-07-28 (v6)** — `NPC.ekJanodGear()` hooked at `Party.a(NPC)`: wand/robes/hood/boots/ring, then `CharacterSheet.ekDropHardcoded()` so gear (not the bestiary `hardcoded_weapon`/`hardcoded_defense`) drives his damage and armour. Self-heals a Janod already stored in an existing save. See `deobf/COMPANION_SPEC.md` §7 |
