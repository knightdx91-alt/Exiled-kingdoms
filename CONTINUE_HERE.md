...
Minimap hook now calls `EkMp.drawPeerMapNames` (name in the player's colour, outlined, where the pin was; others
listed "Name: Area"); world map draws the name only (no square). `deobf/MULTIPLAYER_PORT_SPEC.md` §v68.
dist: base + v68 + v67.
...
Owner: host and guest couldn't attack each other. The PvP setting only applied when toggled mid-session and
the join-time send to guests was never hooked. Host now reads its saved setting; EKPVP resent every 3 s;
default ON (toggle in My address). `deobf/MULTIPLAYER_PORT_SPEC.md` v56. dist: base + v56 + v55.
...
Owner: both devices on mobile data got "couldn't open the port (SocketTimeoutException)". A router found
earlier on Wi-Fi was being called over mobile data. EkNat now always rediscovers, and on mobile data (no
Wi-Fi/Ethernet address) says plainly that carriers block incoming connections and to use Tailscale/ZeroTier.
`deobf/NAT_UPNP_SPEC.md` v54. dist: base + v54 + v53.
...
Owner test: joining the house's own public address from inside the house was refused (router without
NAT loopback). EkNat.resolveJoin asks the router which LAN device owns that port and connects there.
Lobby: sessions + players side by side, taller diagnostics strip, chat takes the freed space (B65).
`deobf/NAT_UPNP_SPEC.md` v50. dist: base + v50 + v49.
