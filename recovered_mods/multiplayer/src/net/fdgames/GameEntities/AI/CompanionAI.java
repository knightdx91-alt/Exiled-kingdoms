...
public class CompanionAI extends AI {
    ...
    public final void a(AI.NPCState nPCState) {
        ...
        if (iOrdinal == 1) {
            ...
            this.state = AI.NPCState.f3193f;
        }
    }

    @Override // net.fdgames.GameEntities.AI.AI
    public final void c(int i2) {
        MapActor mapActorResolveCompanionAnchor;
        Trap trap;
        this.npc = i2;
        NPC npc = (NPC) GameLevel.g(i2);
        if (npc == null || this.state == null || (mapActorResolveCompanionAnchor = LanGameBridge.resolveCompanionAnchor(npc)) == null) {
            return;
        }
        int iS = b.s(mapActorResolveCompanionAnchor.B(), npc.B());
        boolean z2 = npc.wait;
        if (!z2 && iS > 384) {
            npc.f3307x = mapActorResolveCompanionAnchor.f3307x + 32;
            npc.f3308y = mapActorResolveCompanionAnchor.f3308y + 32;
            return;
        }
        if (!z2 && iS > 224) {
            npc.p0(mapActorResolveCompanionAnchor.B());
        }
            ...
            if (npc.sheet.skillSet.g("trap_master") > 0) {
                ...
                while (true) {
                    if (i3 >= GameLevelData.z().size()) {
                        ...
                        if (Math.abs(GameLevelData.z().get(i3).f3307x - i7) < 128 && Math.abs(GameLevelData.z().get(i3).f3308y - i8) < 128 && GameLevelData.z().get(i3).T() && GameLevelData.z().get(i3).s()) {
        ...
        if (GameLevel.g(this.targetActorID).d0() == MapActor.ActorState.f3289e) {
            a(nPCState);
            return;
        }
        if (npc.g0()) {
