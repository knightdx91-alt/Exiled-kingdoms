...
public class Skills {
    ...
    private static ArrayList<Skill> f3501a = new ArrayList<>();

    public static ArrayList<Skill> a() {
        ArrayList<Skill> arrayList = new ArrayList<>();
        for (Skill skill : f3501a) {
            if (skill.advanced) {
                arrayList.add(skill);
            }
        }
    ...
    public static int d(int i2, String str) {
        int i3;
        Iterator<Skill> it = f3501a.iterator();
        while (true) {
            i3 = 0;
            if (!it.hasNext()) {
                break;
    ...
    public static void e() {
        f3501a.clear();
        f(lxYOBQSyWPCj.SkVYg, false);
        f(EGEEZWr.EmwiPdJBV, true);
        f("skills2.txt", false);
        f("skills_advanced2.txt", true);
        f("skills3.txt", false);
        f(ZeMa.aDLqMVBVrSynBEF, true);
    }
    ...
    */
    private static void f(String str, boolean z2) {
        ...
        String[] strArrSplit2 = Gdx.files.internal("data/rules/".concat(str)).readString("UTF-8").replaceAll(lRWXeT.EmienTIPjMCHJhM, "").split("\n");
        String strL = FDUtils.l();
        Skill skill = null;
        if (!strL.equals("")) {
            if (Gdx.files.internal("data/rules/" + strL + str).exists()) {
                strArrSplit = Gdx.files.internal("data/rules/" + strL + str).readString("UTF-16").replace(jzidqMPaLNVH.uBKB, "").replace("\n\r\n\r", "").split("\n");
        ...
        while (true) {
            ...
            if (Settings.h() == 2) {
                ...
                if (str6.equals("") || skill == null) {
                    ...
                    str5 = strArrSplit3[1];
                    if (str5.toLowerCase(locale).equals(fNqLzGyLIBqug.fFgZzno)) {
                        str5.toLowerCase(locale).equals("p");
                        skill_type = Skill.SKILL_TYPE.f3499c;
                    } else {
                        skill_type = Skill.SKILL_TYPE.f3498b;
                    }
