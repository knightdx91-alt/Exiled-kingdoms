...
public class WorldEvents {
    ...
    private static ArrayList<WorldEvent> f3419a = new ArrayList<>();

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    public static ArrayList<EventLocation> f3420b = new ArrayList<>();

    public WorldEvents() {
        int i2;
        f3420b = new ArrayList<>();
        String[] strArrSplit = Gdx.files.internal(fNqLzGyLIBqug.hQStTPN).readString().split("\n");
        int i3 = 0;
        while (true) {
            ...
            f3420b.add(eventLocation);
            i3++;
        }
        f3419a = new ArrayList<>();
        ...
        for (i2 = 1; i2 < strArrSplit3.length; i2++) {
            ...
            f3419a.add(new WorldEvent(strReplace4));
        }
    }

    public static WorldEvent a(String str) {
        for (WorldEvent worldEvent : f3419a) {
    ...
    public static String b(int i2, String str) {
        String str2 = vIBRkbZbNjpf.zlJwN;
        if (i2 == 0) {
            return GameString.b(str2, false);
        }
        String strSubstring = str.substring(0, str.length() - 2);
        if (strSubstring.equals(gNaSiQJmMEn.KTWTUTEvZlpjHj)) {
            strSubstring = "h11_cave_1";
        }
        if (i2 == 1) {
            String strB = GameString.b("LOCATION_REGION", false);
            Areas areas = GameWorld.f3413f;
            Area areaB = areas.b(strSubstring);
            if (strSubstring.contains(fIsXh.pmgQFXBn)) {
                areaB = areas.b(strSubstring.split("\\_")[0]);
            }
            return strB.replace("##REGION##", areaB.c());
        }
        if (i2 == 2) {
            return GameWorld.f3413f.a(strSubstring);
        }
        for (EventLocation eventLocation : f3420b) {
            if (eventLocation.id.equals(str)) {
                String str3 = eventLocation.knowledge;
                if (str3.equals("")) {
                    return GameWorld.f3413f.a(strSubstring);
                }
                return GameWorld.f3413f.a(strSubstring) + ", " + str3;
            }
        }
        return GameString.b(str2, false);
    }
