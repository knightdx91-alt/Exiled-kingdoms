...
public class Quest {
    ...
    public Quest(String str) {
        ...
        this.states = new ArrayList<>();
        d dVar = Gdx.files;
        String str2 = KOdB.aTXJm;
        String[] strArrSplit = dVar.internal(str2 + str + ".txt").readString("UTF-8").split("\n");
        String strL = FDUtils.l();
        String[] strArr = null;
        if (!strL.equals("")) {
            if (Gdx.files.internal(str2 + strL + str + ".txt").exists()) {
                String[] strArrSplit2 = Gdx.files.internal(str2 + strL + str + ".txt").readString("UTF-16").replace("\r\n\r\n", "").replace("\n\r\n\r", "").split("\n");
                ...
                if (strArrSplit2 == null || strArrSplit2[0].toLowerCase(Locale.ENGLISH).contains("trans")) {
                    ...
                    StringBuilder sbT = a.t("WARNING: quest localized file format error, ", str, " ; first text=");
                    sbT.append(strArrSplit2[0]);
