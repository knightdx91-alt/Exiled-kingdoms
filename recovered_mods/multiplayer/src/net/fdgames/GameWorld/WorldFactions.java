...
public class WorldFactions {
    ...
    private static GameText[] f3421a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    private static GameText[] f3422b;
    ...
    public WorldFactions() {
        ...
        int i2 = 1;
        f3421a = new GameText[strArrSplit.length - 1];
        for (int i3 = 1; i3 < strArrSplit.length; i3++) {
            f3421a[i3 - 1] = new GameText(strArrSplit[i3], true, false);
        }
        f3422b = new GameText[strArrSplit.length - 1];
        for (int i4 = 1; i4 < strArrSplit.length; i4++) {
            f3422b[i4 - 1] = new GameText(strArrSplit[i4], true, true);
        }
    ...
    private static int h(String str) {
        for (WorldFaction worldFaction : GameWorld.f3410c.factionList) {
            if (worldFaction.id.equals(str)) {
                return worldFaction.code.intValue();
            }
        }
        System.out.println("caution, faction " + str + KOdB.WQnbaEbfsNIMDP);
    ...
    public static void j() {
        WorldFaction next;
        Iterator<WorldFaction> it = GameWorld.f3410c.factionList.iterator();
        while (true) {
            if (it.hasNext()) {
                next = it.next();
                if (next.code == 100) {
    ...
    public final void a(int i2, int i3, String str) {
        for (WorldFaction worldFaction : this.factionList) {
            if (worldFaction.id.equals(str) && worldFaction.e() < i3) {
                ...
                gameLog.getClass();
                String str2 = SkCylVE.MWXCFCxzYngjtdH;
                if (i2 > 0) {
                    gameLog.a("[GREEN]" + GameString.b("REPUTATION_WITH", false) + " " + strC + " " + GameString.b("REPUTATION_CHANGED_BY", false) + " +" + i2 + str2);
                }
                if (i2 < 0) {
                    gameLog.a("[RED]" + GameString.b("REPUTATION_WITH", false) + " " + strC + " " + GameString.b("REPUTATION_CHANGED_BY", false) + " " + i2 + str2);
                }
