    ...
    public Conversation(String str) {
        ...
        if (!strL.equals("")) {
            if (Gdx.files.internal("data/conversations/" + strL + str2 + ".txt").exists()) {
                String[] strArrSplit2 = Gdx.files.internal("data/conversations/" + strL + str2 + ".txt").readString("UTF-16").split("\r");
                if (strArrSplit2 != null && strArrSplit2.length < strArrSplit.length) {
                    PrintStream printStream = System.out;
                    StringBuilder sbU = a.u("WARNING: conversation localized file length mismatch, ", str2, " ; ");
                    sbU.append(strArrSplit2.length);
                    sbU.append(" vs ");
                    sbU.append(strArrSplit.length);
                    printStream.println(sbU.toString());
                }
                if (strArrSplit2 != null && !strArrSplit2[0].toLowerCase(Locale.ENGLISH).contains("trans")) {
                    PrintStream printStream2 = System.out;
                    StringBuilder sbU2 = a.u("WARNING: conversation localized file format error, ", str2, " ; first text=");
                    sbU2.append(strArrSplit2[0]);
                ...
                if (strArrSplit2 != null) {
                    ...
                    while (true) {
                        ...
                        if (strArrSplit[i3].replace("\n", "").replace("\r", "").split("\t", -1)[2].trim().equals("")) {
                            length = i3;
                            break;
                        }
                        i3++;
        ...
        while (i5 < strArrSplit.length) {
            ...
            if (Settings.h() == 2) {
                ...
                if (strReplace3.trim().equals("")) {
                    System.out.println("WARNING: conversation localized file blank line, " + str2 + " ; line=" + i5);
