...
| A4 | **Particle effects** | Pooled map particles (fire, sparkle, water, spell FX); `nocturne` ones only at night. | **PORTED (spell FX):** `src/particles.js` parses the real libgdx `.p` effects (extracted from the APK, `assets/particle/`) into Phaser 3.80 emitters and plays them as one-shot impact bursts on spell cast (fire/ice/spell/toxic/death/healing/casting). Still absent: ambient **map** particles (bonfires/water/sparkle placed by map objects), `nocturne` night-only gating. `SPEC`: `deobf/SKILLS_EXEC_SPEC.md`. | Effect format ported; map-placed emitters not yet wired. |
...
| A8 | **Combat resolution** | Full `Character.R0` skill/proc/status pipeline (fury, mage armor, bleeds, secondary-damage weapons, per-skill effects). | Weapon roll + crit + exact `g0`/armor mitigation, PatrollerAI aggro/chase/attack, CompanionAI followers, loot/XP on death, **plus** (from `weapons.txt` / `WeaponStats`): **secondary elemental damage** folded into every hit (mitigated by its own type), and **on-hit procs** — `stun` (level s), `paralyze` (level s), `slow` ((level·2)+3 s, ×0.5 move speed), rolled at the weapon's proc %. Not yet: bleeds/DoT, EMP's robot-only 35-75 hit, exact `sheet.f0` resist scaling on procs. | Core formulas + proc effects reversed (`Character.R0/z1/s0`, `WeaponStats`, `DamageEffect`). `SPEC`: `deobf/COMBAT_SPEC.md`. |
...
| A14 | **Items / inventory / equipment** | Full `Item`/`CharacterInventory` — 12 slots, worn-gear bonuses, procs, `traitBonus`/`attributes` mods, `Requisites`, `OnTake`, scroll effects. | **PORTED (core):** 724 items (`items.json`), 12 equip slots, backpack + quick slots, equip/unequip with class-gating, worn armor/hp/mana/resist + shield fold into derived stats & combat, loot drops become real items, `GiveItem/RemoveItem` + consumable `OnUse` (GainHP/GainMana), character/inventory screen. Not yet: item **procs**, trait/attribute item mods, `Requisites`, `OnTake`; **icons now shipped** (426 real item icons in `assets/ui/items/`, wired via `HUD.itemIconHtml`), and window/button frames use the real extracted `uiskin` 9-patch art (`assets/ui/skin/`). Not yet: item **procs**, trait/attribute item mods, `Requisites`, `OnTake`. | Item model + slots + equip rules reversed; art shipped. `SPEC`: `deobf/INVENTORY_SPEC.md`. |
| A15 | **Skill / spell execution** | Every skill's bespoke `SkillActions`/`Character` logic (summons, stealth, traps, all masteries, procs, projectile travel + FX). | **PORTED (representative set):** per-skill-rank effects with mana-spend + cooldowns — heals, 4 caster spells (fireball/lightning/ice-storm/sacred-fire, AoE + stun + undead bonus), warrior/rogue melee actives (whirlwind/charge/bash/kick/stab), self-buffs (resilience/holy-shield/mage-armor/arbenos/evasion), passives (fury, mana_surge); HUD skill bar to cast. Numbers reversed from `SkillActions` + skill texts. Skills outside the table are known-but-inert; summons/stealth/traps/most masteries/procs not modelled. **Projectile + impact FX now PORTED** — real EK projectile atlas art (`assets/sprites/projectiles/`) flies caster→target and the type's real libgdx particle cloud plays on impact (A4). | Effect numbers reversed; long tail APPROX. `SPEC`: `deobf/SKILLS_EXEC_SPEC.md`. |
| A16 | **Attack animation** | `AnimationSet.ActorState.Attack` = sheet rows 6-10 (1-indexed) × 4 frames (cols 1-4), played once at ~0.35s per swing; ranged uses `_ranged` sheet; death is row 11. | **PORTED (melee):** hero + humanoid NPCs play the 4-frame attack row for their facing on every swing (`src/sprite.js` `playAttack`, wired into `combat.js _resolveAttack`); sheets without the attack rows fall back to the old lunge tween. **Ranged attacks now PORTED:** a weapon with the `ranged` flag flies its real projectile sprite (`weapons.txt` `sprite` col → projectiles atlas, else `arrow1`) from attacker to target and lands the damage/secondary/proc on arrival (`Combat._projectileTo`); wands/staves/bows all fire. Still APPROX: dedicated `_ranged` sheet frames + cast/death animation states. | Row layout reversed from `AnimationSet.java`; projectile path from `Character`/`Projectile`. |
